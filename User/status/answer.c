...
void answer3(struct Status *sta) {
  step_clear(&sta->step);
  step_push(&sta->step, action_keep_0, condition_findline_with_80000_90000_history_limit);
  step_push(&sta->step, action_follow, condition_roadless);
  step_push(&sta->step, action_keep_257, condition_findline_with_80000_90000_history_limit);
  step_push(&sta->step, action_follow, condition_roadless);
  step_push(&sta->step, action_stop, condition_never);
}
...
void answer4(struct Status *sta) {
  ...
  for (int i = 0; i < 4; i++) {
    step_push(&sta->step, action_keep_0, condition_findline_with_80000_90000_history_limit);
    step_push(&sta->step, action_follow, condition_roadless);
    step_push(&sta->step, action_turn_to_257, condition_turn_to);
    step_push(&sta->step, action_keep_257, condition_findline_with_80000_90000_history_limit);
    step_push(&sta->step, action_follow, condition_roadless);
