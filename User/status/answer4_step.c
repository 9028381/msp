...
struct SpeedCache speed_cache = {.left = 0, .right = 0};
...
void speed_cache_rev_recover(struct Status *sta) {
  sta->wheels[FONT_LEFT].target = speed_cache.right;
  sta->wheels[FONT_RIGHT].target = speed_cache.left;
}
